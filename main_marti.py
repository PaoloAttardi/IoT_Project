...
def bowlConfig(zone, id, lat, lon):
    ...
    # Prima creiamo la ciotola come prima
    confBowl = Bowl(zone, id, lat, lon)

    # Verifica se l'ID della ciotola non è già attivo
    if confBowl.id not in activeBowls:
        activeBowls[confBowl.id] = confBowl

        # Ottieni una connessione attiva con l'Arduino tramite il Bridge
        bridge = Bridge(port)

        # Verifica che ci sia un Arduino collegato
        if bridge.ser:
            # Invia i dati della ciotola all'Arduino
            bridge.send_bowl_data(zone, id, lat, lon)

            # Attendi un messaggio di conferma dall'Arduino
            response = bridge.ser.read()
            if response == b'\x01':  # Supponiamo che '\x01' sia il messaggio di OK
                print("Dati ricevuti dall'Arduino, avvio della trasmissione dei sensori.")
                return jsonify({"message": "Configurazione ricevuta dall'Arduino"}), 200
            else:
                return jsonify({"error": "Errore durante l'invio dei dati all'Arduino"}), 500
        else:
            return jsonify({"error": "Nessun Arduino collegato"}), 500
    else:
        return jsonify({"message": f"Bowl {confBowl.id} già configurata"}), 200
